...
def  main():
  ...
  installer = "\installer\RCEveryGain Installer.exe"
   
  if demo:
    installer = "\installer\RCEveryGain Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\RCEveryGain manual.pdf" 
  ]
  ...
  ZIPNAME = "RCEveryGain-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "RCEveryGain-v" + FULLVERSIONSTR + "-win-demo.zip"
